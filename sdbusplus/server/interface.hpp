...
struct interface final {
  ...
  interface(sdbusplus::bus::bus& bus, const char* path, const char* interf,
      ...
      : _bus(bus.get()), _path(path), _interf(interf), _slot(nullptr) {
    sd_bus_slot* slot = nullptr;
    sd_bus_add_object_vtable(_bus.get(), &slot, _path.c_str(), _interf.c_str(),
                             vtable, context);
  ...
  void property_changed(const char* property) {
    sd_bus_emit_properties_changed(_bus.get(), _path.c_str(), _interf.c_str(),
                                   property, nullptr);
  }
