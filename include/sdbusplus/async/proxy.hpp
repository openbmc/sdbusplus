...
struct proxy : private sdbusplus::bus::details::bus_friend
{
    ...
    constexpr proxy() requires(!S && !P && !I) = default;
    constexpr proxy() requires(S || P || I) = delete;
        ...
        requires((S) && (P) && (I))
    {
        ...
        return prop_intf.template call<result_t>(ctx, "Get", c_str(i),
                                                 property.data()) |
               execution::then([](auto&& v) { return std::get<T>(v); });
    }
     ...
     */
    template <typename V>
    auto get_all_properties(context& ctx) const requires((S) && (P) && (I))
    {
