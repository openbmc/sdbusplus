...
// deferred_reply, without using a coroutine.
//
// The server registers "DeferredAdd" with register_deferred_method.  The
...
// callback-style async API) and moves the deferred_reply into that
...
namespace
{
constexpr auto service = "xyz.openbmc_project.deferred-example";
constexpr auto path = "/xyz/openbmc_project/deferred";
constexpr auto interface = "xyz.openbmc_project.deferred";
...
// while a DeferredAdd reply is pending.
...
int server()
{
    ...
    // DeferredAdd takes an int32 and returns an int32, but the result is not
    // available when the handler returns.  The handler captures io so it can
    // schedule the async continuation, then moves the deferred_reply into
    // that continuation and returns void.  No reply is sent yet.
    iface->register_deferred_method<int32_t>(
        "DeferredAdd",
        [&io](sdbusplus::asio::deferred_reply reply, int32_t value) {
            std::cout << "server: DeferredAdd(" << value
            ...
            work->async_wait([work, reply = std::move(reply), value](
...
int client()
{
    ...
    conn->async_method_call(
        [](boost::system::error_code ec, int32_t value) {
            if (ec)
            {
                std::cerr << "client: DeferredAdd failed: " << ec << "\n";
                return;
            }
            std::cout << "client: DeferredAdd(41) -> " << value << "\n";
        },
        service, path, interface, "DeferredAdd", int32_t(41));
