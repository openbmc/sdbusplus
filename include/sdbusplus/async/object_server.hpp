...
struct method_instance : method_base
{
    ...
    // The handler coroutine is lazy: it runs after the argument tuple built
    // in spawn_invoke is gone, so reference parameters would dangle.
  ...
  private:
    auto invoke_handler(message_t& m, dbus_args_t args)
    {
        ...
        {
            // unpack_args consumed the shared read cursor; rewind so the
            // handler can unpack the message itself.
...
class dbus_interface : private context_ref
{
  public:
    ...
    bool set_property(const std::string& name, T value)
    {
        ...
        // Owner-side writes go through the same hook as external ones so
        // hook-enforced invariants hold on both paths.
        if (p->on_set)
        {
    ...
    T get_property(const std::string& name) const
    {
        ...
        if (it == _properties.end())
        {
            throw std::out_of_range(
                "dbus_interface: unknown property '" + name + "'");
        }
    ...
    template <typename... Args>
    void emit_signal(const std::string& name, Args&&... args)
    {
        assert_initialized();
        ...
        {
            throw std::out_of_range(
                "dbus_interface: unknown signal '" + name + "'");
        }
