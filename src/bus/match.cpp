...
static slot makeMatch(SdBusInterface* intf, sd_bus* bus, const char* _match,
                      sd_bus_message_handler_t handler, void* context)
{
    sd_bus_slot* slotp;
    int r = intf->sd_bus_add_match(bus, &slotp, _match, handler, context);
    if (r < 0)
    {
        throw exception::SdBusError(-r, "sd_bus_match");
    }
    return slot{slotp, intf};
}
