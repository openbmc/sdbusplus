...
class dbus_interface
{
  public:
    ...
    bool register_signal(const std::string& name)
    {
        ...
        auto nameItr = signalNames_.emplace(signalNames_.end(), name);
        vtable_.emplace_back(vtable::signal(nameItr->c_str(), ""));
        return true;
    }
    ...
    bool register_signal(const std::string& name)
    {
        ...
        vtable_.emplace_back(
            vtable::signal(nameItr->c_str(), signature.data()));
  ...
  private:
    ...
    std::list<std::string> signalNames_;
