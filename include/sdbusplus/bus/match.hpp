...
constexpr auto argN(size_t n, std::string_view s)
{
    stdplus::ToStrHandle<stdplus::ToStr<size_t>> tsh;
    return strCat("arg"sv, tsh(n), "='"sv, s, "',"sv);
}
constexpr auto argNpath(size_t n, std::string_view s)
{
    stdplus::ToStrHandle<stdplus::ToStr<size_t>> tsh;
...
constexpr auto propertiesChanged(std::string_view p, std::string_view i)
{
 ...
 */
constexpr auto nameOwnerChanged(std::string_view s)
{
