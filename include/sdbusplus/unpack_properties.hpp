...
template <bool Noexcept, typename OnErrorCallback, typename VariantType,
          typename ValueType>
inline bool readProperty(
    const OnErrorCallback& onErrorCallback,
    const std::vector<std::pair<std::string, VariantType>>& container,
    const std::string& expectedKey, ValueType& outValue) noexcept(Noexcept)
{
...
template <bool Noexcept, size_t Index, typename OnErrorCallback,
...
{
    if (!readProperty<Noexcept>(onErrorCallback, container, expectedKey,
                                outValue))
    {
        return false;
    }
    ...
    {
        return readProperties<Noexcept, Index + 1>(
...
template <bool Noexcept = true, typename OnErrorCallback, typename VariantType,
          typename... Args>
    ...
    Args&&... args) noexcept(Noexcept)
{
    ...
    return details::readProperties<Noexcept, 0>(
...
inline void unpackProperties(
    ...
    Args&&... args)
{
    details::unpackPropertiesCommon<false>(
