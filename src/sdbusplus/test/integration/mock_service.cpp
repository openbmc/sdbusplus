...
void MockService::addObject(std::shared_ptr<MockObject> obj)
{
    ...
    if (isStarted())
    {
        std::lock_guard<std::mutex> lock(afterStartPendingObjectsMutex);
        afterStartPendingObjects.push_back(obj->getPath());
    }
}
...
{
    if (!afterStartPendingObjects.empty())
    {
        std::lock_guard<std::mutex> lock(afterStartPendingObjectsMutex);
        for (const auto& path : afterStartPendingObjects)
        {
            objectRepo[path]->start(*bus);
        }
        afterStartPendingObjects.clear();
    }
}
