...
- serviceName
  - A default service name which can host this D-Bus interface.
...
A service name must have the YAML property `name` & `default` and may optionally
have a `description`. The service name shall be specified only for interfaces
which will be implemented by a single service and are singletons, for example,
ObjectMapper is one such case.
...
service_names:
  - name: BlackJack
    description: >
      The service name for the black jack game manager.
    default: com.openbmc_project.black_jack
```
