 ...
 */
template <typename... Args> void append(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Args&&... args);
...
template <typename... Args> struct append_single<std::tuple<Args...>>
{
    ...
    template <typename S> static void op(sdbusplus::SdBusInterface* intf, sd_bus_message* m, S&& s)
    {
        ...
        intf->sd_bus_message_open_container(m, SD_BUS_TYPE_STRUCT, dbusType.data());
...
void append_tuple(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Tuple&& t, std::index_sequence<I...>)
{
    auto dbusTypes =
...
template <typename T>
static void tuple_item_append(sdbusplus::SdBusInterface* intf, sd_bus_message* m, T&& t) {
...
struct AppendHelper {
  ...
  static void op(sdbusplus::SdBusInterface* intf, sd_bus_message* m, std::tuple<Fields...> field_tuple) {
    AppendHelper<Index - 1>::op(intf, m, field_tuple);

    tuple_item_append(intf, m, std::get<Index - 1>(field_tuple));
  }
};
    ...
    append_tuple(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Tuple&& t)
{
    ...
    append_single_t<itemType>::op(intf, m, std::forward<itemType>(std::get<0>(t)));
}
...
std::enable_if_t<
    ...
    append_grouping(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Tuple&& t, Arg&& arg);
/** @brief Group a sequence of C++ types for appending into an sd_bus_message.
 *  @tparam Tuple - A tuple of previously analyzed types.
 *  @tparam Arg - The argument to analyze for grouping.
 *
...
std::enable_if_t<
    can_append_multiple<types::details::type_id_downcast_t<Arg>>::value>
    append_grouping(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Tuple&& t, Arg&& arg, Rest&&... rest);
/** @brief Group a sequence of C++ types for appending into an sd_bus_message.
 *  @tparam Tuple - A tuple of previously analyzed types.
 *  @tparam Arg - The argument to analyze for grouping.
 *  @tparam Rest - The remaining arguments left to analyze.
...
template <typename... Args> void append(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Args&&... args)
{
    details::append_grouping(intf, m, std::make_tuple(), std::forward<Args>(args)...);
}
