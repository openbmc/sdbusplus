...
class MethodCall<Types<>, Rets>
{
  public:
    ...
    template <template <typename> typename Promise = stdplus::util::Promise,
              typename... Args>
    auto call_async_future(Args&&... args) &&
    {
        Promise<MethodCall<void, Rets>> p;
...
template <>
class MethodCall<Types<>, void> : public Sender
{
  public:
    MethodCall(message::message&& m) : Sender(std::move(m))
    {
    }
};
