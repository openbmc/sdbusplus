...
#include <sdbusplus/async/std_execution/__utility.hpp>
#include <sdbusplus/async/std_execution/concepts.hpp>
#include <sdbusplus/async/std_execution/coroutine.hpp>
#include <sdbusplus/async/std_execution/functional.hpp>
#include <sdbusplus/async/std_execution/stop_token.hpp>
...
struct __with_x
{
    struct __t
    {
        ...
        friend auto tag_invoke(
            same_as<_Tag> auto, const __t& __self,
            auto&&...) noexcept(std::
...
struct __with_x<_Tag, __none_such>
{
    struct __t
    {
        ...
        friend void tag_invoke(same_as<_Tag> auto, const __t& __self,
                               auto&&...) = delete;
    };
};
...
struct __env_ : _Withs...
{
    ...
    template <
        __none_of<typename _Withs::__tag_t..., get_completion_signatures_t>
            _Tag,
        same_as<__env_> _Self, class... _As>
    requires __callable<_Tag, const typename _Self::__base_env_t&, _As...>
    friend auto tag_invoke(_Tag __tag, const _Self& __self,
                           _As&&... __as) noexcept
        -> __call_result_if_t<same_as<_Self, __env_>, _Tag,
                              const typename _Self::__base_env_t&, _As...>
    {
...
struct __make_env_t
{
    ...
    template <class... _Ws>
    auto operator()(no_env, __with_<_Ws>...) const noexcept -> no_env
    {
        return {};
    }
};
...
template <class... _Ts>
using make_env_t = decltype(make_env(__declval<_Ts>()...));
...
using dependent_completion_signatures = __completion_signatures::__dependent;
...
struct completion_signatures
{
    ...
    using __gather_sigs =
        __minvoke<__concat<_Variant>, __signal_args_t<_Sigs, _Tag, _Tuple>...>;
};
...
{
    using __t = __minvoke<__concat<__munique<__q<completion_signatures>>>,
                          _Completions...>;
};
...
concept __with_member_alias = requires
{
...
struct get_completion_signatures_t
{
    ...
    requires(__with_tag_invoke<_Sender, _Env> || __with_member_alias<_Sender> ||
             __awaitable<_Sender>) constexpr auto
        operator()(_Sender&&, const _Env& = {}) const noexcept
    {
        ...
        else
        {
            ...
            {
                return completion_signatures<set_value_t(_Result),
                                             set_error_t(std::exception_ptr)>{};
            }
        }
    }
...
template <class _Sender, class _Env = no_env>
concept sender = __sender<_Sender, _Env> && __sender<_Sender, no_env> &&
...
using __nullable_variant_t =
    __munique<__mbind_front<__q<std::variant>, std::monostate>>;
...
requires sender<_Sender, _Env>
using __gather_sigs_t = typename completion_signatures_of_t<
    _Sender, _Env>::template __gather_sigs<_Tag, _Tuple, _Variant>;
...
concept __single_typed_sender =
    sender<_Sender, _Env> && __valid<__single_sender_value_t, _Sender, _Env>;
...
template <class _Sender, class _Env = no_env>
concept __single_value_variant_sender = sender<_Sender, _Env> &&
...
using __ensure_concat =
    __minvoke<__concat<__q<completion_signatures>>, _Sigs...>;
...
struct get_forward_progress_guarantee_t
{
    ...
    constexpr tag_invoke_result_t<get_forward_progress_guarantee_t,
                                  __cref_t<_T>>
        operator()(_T&& __t) const
        noexcept(nothrow_tag_invocable<get_forward_progress_guarantee_t,
                                       __cref_t<_T>>)
    {
        return tag_invoke(get_forward_progress_guarantee_t{},
                          std::as_const(__t));
    }
    constexpr execution::forward_progress_guarantee
...
concept scheduler = requires(
    _Scheduler&& __sched,
    const __sender_queries::get_completion_scheduler_t<set_value_t> __tag)
{
    {
        schedule((_Scheduler &&) __sched)
        } -> sender;
    {
        tag_invoke(__tag, schedule((_Scheduler &&) __sched))
...
struct __promise : __promise_base
{
    ...
    explicit __promise(auto&, _Receiver& __rcvr) noexcept : __rcvr_(__rcvr) {}
    ...
    auto await_transform(_Awaitable&& __await) noexcept(
        nothrow_tag_invocable<as_awaitable_t, _Awaitable, __promise&>)
...
struct __connect_awaitable_t
{
  ...
  public:
    ...
    operator()(_Awaitable&& __await, _Receiver&& __rcvr) const
    {
...
template <class _Env>
using __debug_env_t = make_env_t<_Env, with_t<__is_debug_env_t, bool>>;
...
{
    friend void tag_invoke(set_value_t, __any_debug_receiver&&,
                           auto&&...) noexcept;
    friend void tag_invoke(set_error_t, __any_debug_receiver&&,
                           auto&&) noexcept;
    friend void tag_invoke(set_stopped_t, __any_debug_receiver&&) noexcept;
    friend __debug_env_t<_Env> tag_invoke(get_env_t,
                                          __any_debug_receiver) noexcept;
};

struct connect_t;

template <class _Sender, class _Receiver>
concept __connectable_with_tag_invoke = sender<_Sender, env_of_t<_Receiver>> &&
...
concept sender_of = sender<_Sender, _Env> &&
    same_as<__types<_SetSig>,
            __gather_sigs_t<__tag_of_sig_t<_SetSig>, _Sender, _Env,
...
struct __receiver_base
{
    ...
    template <class _Error>
    friend void tag_invoke(set_error_t, __receiver_base&& __self,
        ...
        __single_sender_value_t<__t<_SenderId>, env_of_t<__t<_PromiseId>>>>
{
  ...
  public:
    __sender_awaitable(
        ...
        __op_state_(connect((_Sender &&) sender,
...
using __sender_awaitable_t = __sender_awaitable<__x<_Promise>, __x<_Sender>>;
...
using __receiver = typename __sender_awaitable_base<
    __x<_Promise>,
    __single_sender_value_t<_Sender, env_of_t<_Promise>>>::__receiver;
...
struct as_awaitable_t
{
        ...
        noexcept(__is_noexcept<_T, _Promise>())
    {
        ...
        else if constexpr (__impl::__awaitable_sender<_T, _Promise>)
        {
            ...
            return __impl::__sender_awaitable_t<_Promise, _T>{(_T &&) __t,
                                                              __hcoro};
        }
        else
        {
...
{
    friend void tag_invoke(set_value_t, __detached_receiver&&,
                           auto&&...) noexcept
    {}
    [[noreturn]] friend void tag_invoke(set_error_t, __detached_receiver&&,
                                        auto&&) noexcept
    {
        std::terminate();
    }
    friend void tag_invoke(set_stopped_t, __detached_receiver&&) noexcept {}
    friend __empty_env tag_invoke(get_env_t,
                                  const __detached_receiver&) noexcept
    {
        return {};
    }
};
...
struct start_detached_t
{
    ...
    template <sender _Sender>
    requires(!tag_invocable<start_detached_t, _Sender>) &&
        sender_to<_Sender, __impl::__detached_receiver> void
        operator()(_Sender&& __sndr) const noexcept(false)
    {
...
struct __sender
{
    ...
    template <class _Receiver>
    requires(copy_constructible<_Ts>&&...) friend auto tag_invoke(
        connect_t, const __sender& __sndr,
        _Receiver&&
    ...
    template <class _Receiver>
    friend auto
        tag_invoke(connect_t, __sender&& __sndr, _Receiver&& __rcvr) noexcept(
...
struct __as_receiver
{
    ...
    friend __empty_env tag_invoke(get_env_t, const __as_receiver&) noexcept
    {
        return {};
    };
};
...
struct execute_t
{
    ...
    void operator()(_Scheduler&& __sched, _Fun __fun) const noexcept(
        noexcept(__submit(schedule((_Scheduler &&) __sched),
                          __impl::__as_receiver<_Fun>{(_Fun &&) __fun})))
    {
...
requires __callable<_Closure, _Sender> __call_result_t<_Closure, _Sender>
...
#define _MISSING_MEMBER(_D, _TAG)                                              \
    requires                                                                   \
    {                                                                          \
        typename _D::_TAG;                                                     \
    }
...
struct sender_adaptor
{
    class __t : __adaptor_base<_Base>
    {
        ...
        template <same_as<connect_t> _Connect, __decays_to<_Derived> _Self,
                  receiver _Receiver>
        friend auto
            tag_invoke(_Connect, _Self&& __self, _Receiver&& __rcvr) noexcept(
                noexcept(_CALL_MEMBER(connect, (_Self &&) __self,
                                      (_Receiver &&) __rcvr)))
                -> decltype(_CALL_MEMBER(connect, (_Self &&) __self,
                                         (_Receiver &&) __rcvr))
        {
            return _CALL_MEMBER(connect, (_Self &&) __self,
                                (_Receiver &&) __rcvr);
        }
        ...
        friend auto
            tag_invoke(_Connect, _Self&& __self, _Receiver&& __rcvr) noexcept(
                __nothrow_connectable<__member_t<_Self, _Base>, _Receiver>)
                -> connect_result_t<__member_t<_Self, _Base>, _Receiver>
        {
            return execution::connect(((__t &&) __self).base(),
                                      (_Receiver &&) __rcvr);
        }
        ...
        friend auto tag_invoke(
            _Tag __tag, const _Derived& __self,
            _As&&... __as) noexcept(__nothrow_callable<_Tag, const _Base&,
                                                       _As...>)
            -> __call_result_if_t<tag_category<_Tag, forwarding_sender_query>,
                                  _Tag, const _Base&, _As...>
        {
...
struct receiver_adaptor
{
    class __t : __adaptor_base<_Base>
    {
        ...
        using __get_base_t =
            __if<__bool<__has_base>, __mbind_back<__defer<__member_t>, _Base>,
        ...
        friend void tag_invoke(_SetValue, _Derived&& __self,
                               _As&&... __as) noexcept
        {
            execution::set_value(__get_base((_Derived &&) __self),
                                 (_As &&) __as...);
        }
        ...
        friend void tag_invoke(_SetError, _Derived&& __self,
                               _Error&& __err) noexcept
        {
            execution::set_error(__get_base((_Derived &&) __self),
                                 (_Error &&) __err);
        }
...
struct operation_state_adaptor
{
    class __t : __adaptor_base<_Base>, __immovable
    {
        ...
        friend auto tag_invoke(
            ...
            _As&&... __as) noexcept(__nothrow_callable<_Tag, const _Base&,
                                                       _As...>)
            -> __call_result_if_t<__none_of<_Tag, start_t>, _Tag, const _Base&,
                                  _As...>
        {
            return ((_Tag &&) __tag)(__c_cast<__t>(__self).base(),
                                     (_As &&) __as...);
        }
...
struct scheduler_adaptor
{
    class __t : __adaptor_base<_Base>
    {
        ...
        friend auto tag_invoke(_Schedule, _Self&& __self) noexcept(
            noexcept(_CALL_MEMBER(schedule, (_Self &&) __self)))
        ...
        requires _MISSING_MEMBER(
            decay_t<_Self>, schedule) && scheduler<__member_t<_Self, _Base>>
        friend auto tag_invoke(_Schedule, _Self&& __self) noexcept(noexcept(
            execution::schedule(__declval<__member_t<_Self, _Base>>())))
            -> schedule_result_t<_Self>
        {
            return execution::schedule(__c_cast<__t>((_Self &&) __self).base());
        }

        template <tag_category<forwarding_scheduler_query> _Tag,
                  same_as<_Derived> _Self, class... _As>
        requires __callable<_Tag, const _Base&, _As...>
        friend auto
            tag_invoke(_Tag __tag, const _Self& __self, _As&&... __as) noexcept(
                __nothrow_callable<_Tag, const _Base&, _As...>)
                -> __call_result_if_t<
                    tag_category<_Tag, forwarding_scheduler_query>, _Tag,
                    const _Base&, _As...>
        {
            return ((_Tag &&) __tag)(__c_cast<__t>(__self).base(),
                                     (_As &&) __as...);
                        ...
                        _As&&... __as) noexcept
{
    ...
    {
        (__set_value_invoke_)((_Receiver &&) __rcvr, (_Fun &&) __fun,
                              (_As &&) __as...);
    }
    else
    {
...
using __with_error_invoke_t = __if_c<
    __v<__gather_sigs_t<_Tag, _Sender, _Env,
                        __mbind_front_q<__non_throwing_, _Fun>, __q<__mand>>>,
    completion_signatures<>, __with_exception_ptr>;

template <class _Fun, class... _Args>
requires invocable<_Fun, _Args...>
using __set_value_invoke_t =
    completion_signatures<__minvoke1<__remove<void, __qf<set_value_t>>,
    ...
    receiver_adaptor<__receiver<_ReceiverId, _FunId>, __t<_ReceiverId>>
{
    ...
    {
        (__set_value_invoke)(((__receiver &&) * this).base(), (_Fun &&) __f_,
                             (_As &&) __as...);
    }
...
struct __sender
{
    ...
    using __completion_signatures = __make_completion_signatures<
        __member_t<_Self, _Sender>, _Env,
        __with_error_invoke_t<set_value_t, _Fun, __member_t<_Self, _Sender>,
                              _Env>,
    ...
    friend auto tag_invoke(
        ...
        _Receiver&&
            __rcvr) noexcept(__nothrow_connectable<__member_t<_Self, _Sender>,
    ...
    friend auto
        tag_invoke(_Tag __tag, const __sender& __self, _As&&... __as) noexcept(
            __nothrow_callable<_Tag, const _Sender&, _As...>)
            -> __call_result_if_t<tag_category<_Tag, forwarding_sender_query>,
...
struct then_t
{
    ...
    using __sender =
        __sender<__x<remove_cvref_t<_Sender>>, __x<remove_cvref_t<_Fun>>>;
    ...
    requires(!__tag_invocable_with_completion_scheduler<then_t, set_value_t,
                                                        _Sender, _Fun>) &&
        tag_invocable<then_t, _Sender, _Fun> sender auto
        operator()(_Sender&& __sndr, _Fun __fun) const
    ...
    requires(!__tag_invocable_with_completion_scheduler<then_t, set_value_t,
                                                        _Sender, _Fun>) &&
        (!tag_invocable<then_t, _Sender, _Fun>)&&sender<
            __sender<_Sender, _Fun>> __sender<_Sender, _Fun>
        operator()(_Sender&& __sndr, _Fun __fun) const
    {
...
struct __sender
{
    ...
    using __completion_signatures = __make_completion_signatures<
        __member_t<_Self, _Sender>, _Env,
        __with_error_invoke_t<set_error_t, _Fun, __member_t<_Self, _Sender>,
                              _Env>,
    ...
    friend auto
        tag_invoke(_Tag __tag, const __sender& __self, _Error&& __err) noexcept(
            __nothrow_callable<_Tag, const _Sender&, _Error>)
            -> __call_result_if_t<tag_category<_Tag, forwarding_sender_query>,
...
struct upon_error_t
{
    ...
    requires(!__tag_invocable_with_completion_scheduler<
             upon_error_t, set_error_t, _Sender, _Fun>) &&
        tag_invocable<upon_error_t, _Sender, _Fun> sender auto
        operator()(_Sender&& __sndr, _Fun __fun) const
    ...
    requires(!__tag_invocable_with_completion_scheduler<
             upon_error_t, set_error_t, _Sender, _Fun>) &&
        (!tag_invocable<upon_error_t, _Sender, _Fun>)&&sender<
            __sender<_Sender, _Fun>> __sender<_Sender, _Fun>
        operator()(_Sender&& __sndr, _Fun __fun) const
    {
        return __sender<_Sender, _Fun>{(_Sender &&) __sndr, (_Fun &&) __fun};
...
struct __sender
{
    ...
    using __completion_signatures = __make_completion_signatures<
        __member_t<_Self, _Sender>, _Env,
        __with_error_invoke_t<set_stopped_t, _Fun, __member_t<_Self, _Sender>,
                              _Env>,
        __q<__completion_signatures::__default_set_value>,
    ...
    friend auto
        tag_invoke(connect_t, _Self&& __self, _Receiver&& __rcvr) noexcept(
            ...
            -> connect_result_t<__member_t<_Self, _Sender>,
                                __receiver<_Receiver>>
    {
        return execution::connect(
            ((_Self &&) __self).__sndr_,
    ...
    friend auto tag_invoke(_Tag __tag, const __sender& __self) noexcept(
        __nothrow_callable<_Tag, const _Sender&>)
        -> __call_result_if_t<tag_category<_Tag, forwarding_sender_query>, _Tag,
                              const _Sender&>
    {
...
struct upon_stopped_t
{
    ...
    requires __tag_invocable_with_completion_scheduler<
        ...
        noexcept(nothrow_tag_invocable<
                 upon_stopped_t,
                 __completion_scheduler_for<_Sender, set_stopped_t>, _Sender,
                 _Fun>)
    {
    ...
    requires(!__tag_invocable_with_completion_scheduler<
        ...
        tag_invocable<upon_stopped_t, _Sender, _Fun>&& __callable<_Fun> sender
        auto
        operator()(_Sender&& __sndr, _Fun __fun) const
    ...
    requires(!__tag_invocable_with_completion_scheduler<
             upon_stopped_t, set_stopped_t, _Sender, _Fun>) &&
        (!tag_invocable<upon_stopped_t, _Sender, _Fun>)&&__callable<
            _Fun>&& sender<__sender<_Sender, _Fun>> __sender<_Sender, _Fun>
        operator()(_Sender&& __sndr, _Fun __fun) const
    {
        return __sender<_Sender, _Fun>{(_Sender &&) __sndr, (_Fun &&) __fun};
    }
    ...
    receiver_adaptor<__receiver<_ReceiverId, _Shape, _FunId>, __t<_ReceiverId>>
{
        ...
        __callable<_Fun, _Shape, _As&...>
    {
        ...
        {
            execution::set_error(std::move(this->base()),
...
struct __sender
{
    ...
    template <receiver _Receiver>
    using __receiver =
        __receiver<__x<remove_cvref_t<_Receiver>>, _Shape, _FunId>;
    ...
    using __with_error_invoke_t = __if_c<
        __v<__value_types_of_t<_Sender, _Env,
...
struct bulk_t
{
    ...
    using __sender = __sender<__x<remove_cvref_t<_Sender>>, _Shape,
                              __x<remove_cvref_t<_Fun>>>;

    template <sender _Sender, integral _Shape, __movable_value _Fun>
    requires __tag_invocable_with_completion_scheduler<bulk_t, set_value_t,
                                                       _Sender, _Shape, _Fun>
        sender auto operator()(_Sender&& __sndr, _Shape __shape,
                               _Fun __fun) const
        noexcept(nothrow_tag_invocable<
    ...
    requires(!__tag_invocable_with_completion_scheduler<
        ...
        tag_invocable<bulk_t, _Sender, _Shape, _Fun> sender auto
...
class __receiver
{
  ...
  public:
    ...
    {
        return make_env(with(get_stop_token,
                             __self.__sh_state_.__stop_source_.get_token()));
    }
...
struct __sh_state
{
    ...
    using __bound_values_t = __value_types_of_t<
        _Sender, make_env_t<with_t<get_stop_token_t, in_place_stop_token>>,
        __mbind_front_q<__decayed_tuple, set_value_t>, __q<__bind_tuples>>;
    ...
    void __notify() noexcept
    {
        ...
        void* __old =
            __head_.exchange(__completion_state, std::memory_order_acq_rel);
...
class __operation : public __operation_base
{
  ...
  public:
    __operation(
        _Receiver&& __rcvr,
        std::shared_ptr<__sh_state<_SenderId>>
            __shared_state) noexcept(std::
                                         is_nothrow_move_constructible_v<
                                             _Receiver>) :
        __operation_base{nullptr, __notify},
        __recvr_((_Receiver &&) __rcvr), __shared_state_(move(__shared_state))
    {}
...
class __sender
{
  ...
  public:
    ...
    requires receiver_of<_Receiver,
                         completion_signatures_of_t<_Self, __empty_env>>
    friend auto
        tag_invoke(connect_t, _Self&& __self, _Receiver&& __recvr) noexcept(
            std::is_nothrow_constructible_v<decay_t<_Receiver>, _Receiver>)
    ...
    requires(!__is_instance_of<_Tag, get_completion_scheduler_t>) &&
        __callable<_Tag, const _Sender&, _As...> friend auto tag_invoke(
            _Tag __tag, const __sender& __self,
...
struct split_t
{
    ...
    template <sender _Sender>
    requires __tag_invocable_with_completion_scheduler<split_t, set_value_t,
                                                       _Sender>
        sender auto operator()(_Sender&& __sndr) const
        noexcept(nothrow_tag_invocable<
                 split_t, __completion_scheduler_for<_Sender, set_value_t>,
                 _Sender>)
    {
        auto __sched = get_completion_scheduler<set_value_t>(__sndr);
    ...
    requires(!__tag_invocable_with_completion_scheduler<split_t, set_value_t,
                                                        _Sender>) &&
        tag_invocable<split_t, _Sender> sender auto
        operator()(_Sender&& __sndr) const
    ...
    requires(!__tag_invocable_with_completion_scheduler<split_t, set_value_t,
                                                        _Sender>) &&
        (!tag_invocable<split_t, _Sender>)__sender<_Sender>
        operator()(_Sender&& __sndr) const
    {
...
{
    template <sender _Sender>
    requires __tag_invocable_with_completion_scheduler<ensure_started_t,
                                                       set_value_t, _Sender>
        sender auto operator()(_Sender&& __sndr) const
        noexcept(nothrow_tag_invocable<
                 ensure_started_t,
    ...
    {
        auto __sched = get_completion_scheduler<set_value_t>(__sndr);
        return tag_invoke(ensure_started_t{}, std::move(__sched),
                          (_Sender &&) __sndr);
    }
    template <sender _Sender>
    requires(!__tag_invocable_with_completion_scheduler<
             ensure_started_t, set_value_t, _Sender>) &&
        tag_invocable<ensure_started_t, _Sender> sender auto
        operator()(_Sender&& __sndr) const
...
struct __applyable_fn
{
    ...
    template <class... _As>
    requires invocable<_Fun, _As...> std::invoke_result_t<_Fun, _As...>
    operator()(_As&&...) const
    {
...
concept __applyable = requires(__applyable_fn<_Fun> __fun, _Tuple&& __tupl)
{
    {
        std::apply(__fun, (_Tuple &&) __tupl)
        } -> __none_of<__>;
};
template <class _Fun, class _Tuple>
...
struct __storage
{
    ...
    // sender:
    using __args_t =
        __gather_sigs_t<_SetTag, _Sender, env_of_t<_Receiver>,
                        __q<__decayed_tuple>, __nullable_variant_t>;
    __args_t __args_;

    // Compute a variant of operation states:
    using __op_state3_t =
        __gather_sigs_t<_SetTag, _Sender, env_of_t<_Receiver>,
                        __q<__op_state_for_t>, __nullable_variant_t>;
...
requires(
    !same_as<_Set, _Ret>) struct __tfx_signal_<_Env, _Fun, _Set, _Ret(_Args...)>
{
...
{
    using __t = make_completion_signatures<
        __result_sender_t<_Fun, _Args...>, _Env,
...
struct __receiver
{
    ...
    requires same_as<_Let, set_error_t> &&(
        !__v<__error_types_of_t<
            _Sender, _Env,
            __transform<
                __q1<decay_t>,
                __contains<
                    std::
                        exception_ptr>>>>)friend void tag_invoke(set_error_t,
                                                                 __receiver&&
                                                                     __self,
                                                                 _Error&&
                                                                     __err) noexcept
    ...
    & > &&sender_to<__apply_result_t<_Fun, __which_tuple_t<_As...>&>,
                    _Receiver> friend void
        tag_invoke(_Tag, __receiver&& __self, _As&&... __as) noexcept
    try
    {
        using __tuple_t = __which_tuple_t<_As...>;
        using __op_state_t = __mapply<__q<__op_state_for_t>, __tuple_t>;
        auto& __args =
            __self.__op_state_->__storage_.__args_.template emplace<__tuple_t>(
                (_As &&) __as...);
        auto& __op =
            __self.__op_state_->__storage_.__op_state3_
...
struct __operation
{
    ...
    __operation(_Sender&& __sndr, _Receiver2&& __rcvr, _Fun __fun) :
        ...
        __rcvr_((_Receiver2 &&) __rcvr), __fun_((_Fun &&) __fun)
    {}
    _P2300_IMMOVABLE(__operation);
...
struct __sender
{
    ...
    using __operation_t =
        __operation<__x<__member_t<_Self, _Sender>>,
                    __x<remove_cvref_t<_Receiver>>, _FunId, _Set>;
    template <class _Self, class _Receiver>
    using __receiver_t =
        __receiver<__x<__member_t<_Self, _Sender>>,
                   __x<remove_cvref_t<_Receiver>>, _FunId, _Set>;

    template <class _Sender, class _Env>
    using __with_error = __if_c<__sends<_Set, _Sender, _Env>,
                                __with_exception_ptr, completion_signatures<>>;

    template <class _Sender, class _Env>
    using __completions =
        __mapply<__transform<__tfx_signal<_Env, _Fun, _Set>,
                             __mbind_front_q<__concat_completion_signatures_t,
                                             __with_error<_Sender, _Env>>>,
                 completion_signatures_of_t<_Sender, _Env>>;

    template <__decays_to<__sender> _Self, receiver _Receiver>
    requires sender_to<__member_t<_Self, _Sender>,
...
struct __let_xxx_t
{
    using __t = _SetTag;
    template <class _Sender, class _Fun>
    using __sender = __impl::__sender<__x<remove_cvref_t<_Sender>>,
                                      __x<remove_cvref_t<_Fun>>, _LetTag>;
    ...
    requires(!__tag_invocable_with_completion_scheduler<_LetTag, set_value_t,
                                                        _Sender, _Fun>) &&
        tag_invocable<_LetTag, _Sender, _Fun> sender auto
        operator()(_Sender&& __sndr, _Fun __fun) const
    ...
    requires(!__tag_invocable_with_completion_scheduler<_LetTag, set_value_t,
                                                        _Sender, _Fun>) &&
        (!tag_invocable<_LetTag, _Sender, _Fun>)&&sender<
            __sender<_Sender, _Fun>> __sender<_Sender, _Fun>
        operator()(_Sender&& __sndr, _Fun __fun) const
    {
        return __sender<_Sender, _Fun>{(_Sender &&) __sndr, (_Fun &&) __fun};
...
struct let_value_t : __let::__impl::__let_xxx_t<let_value_t, set_value_t>
{};

struct let_error_t : __let::__impl::__let_xxx_t<let_error_t, set_error_t>
{};

struct let_stopped_t : __let::__impl::__let_xxx_t<let_stopped_t, set_stopped_t>
{};
} // namespace __let
...
struct __receiver : receiver_adaptor<__receiver<_SenderId, _ReceiverId>>
{
    ...
    {
        execution::set_error(((__receiver &&) * this).base(),
                             std::current_exception());
    }
    void set_stopped() && noexcept
    {
...
struct __sender
{
    ...
    template <class... _Tys>
    requires(sizeof...(_Tys) == 1) using __set_value_t =
...
{
    template <sender _Sender>
    auto operator()(_Sender&& __sndr) const -> __sender<__x<decay_t<_Sender>>>
    {
...
struct __operation : __task
{
    ...
    explicit __operation(__task* __tail) noexcept : __task{.__tail_ = __tail} {}
  ...
  public:
    class __scheduler
    {
      ...
      public:
        ...
        friend execution::forward_progress_guarantee
...
{
template <class _ReceiverId>
inline void __operation<_ReceiverId>::__start_() noexcept
try
{
...
using __variant_for_t = __minvoke<__minvoke<
    __fold_right<__nullable_variant_t,
...
struct __receiver2
{
    ...
    friend void tag_invoke(_Tag, __receiver2&& __self,
                    ...
                    _Args&&... __args) noexcept(__nothrow_complete_<_Args...>)
    {
        ...
        __self.__op_state_->__state2_.emplace(
            __conv{[__op_state = __self.__op_state_] {
                return connect(schedule(__op_state->__sched_),
...
struct __operation1
{
    ...
    __operation1(_Scheduler __sched, _CvrefSender&& __sndr,
...
struct __sender
{
    ...
    friend auto tag_invoke(connect_t, _Self&& __self, _Receiver&& __rcvr)
        -> __operation1<_SchedulerId, __x<__member_t<_Self, _Sender>>,
                        __x<decay_t<_Receiver>>>
    {
    ...
    friend _Scheduler tag_invoke(get_completion_scheduler_t<_Tag>,
                                 const __sender& __self) noexcept
    {
    ...
    friend auto tag_invoke(get_completion_signatures_t, _Self&&, _Env)
        -> make_completion_signatures<
            __member_t<_Self, _Sender>, _Env,
            make_completion_signatures<schedule_result_t<_Scheduler>, _Env,
                                       __with_error_t<_Env>, __value_t>>;
};
...
struct transfer_t
{
    ...
    requires __tag_invocable_with_completion_scheduler<transfer_t, set_value_t,
                                                       _Sender, _Scheduler>
    ...
    requires(!__tag_invocable_with_completion_scheduler<transfer_t, set_value_t,
                                                        _Sender, _Scheduler>) &&
        tag_invocable<transfer_t, _Sender, _Scheduler> tag_invoke_result_t<
            transfer_t, _Sender, _Scheduler>
        operator()(_Sender&& __sndr, _Scheduler&& __sched) const
    ...
    requires(!__tag_invocable_with_completion_scheduler<transfer_t, set_value_t,
                                                        _Sender, _Scheduler>) &&
        (!tag_invocable<transfer_t, _Sender, _Scheduler>)auto
        operator()(_Sender&& __sndr, _Scheduler&& __sched) const
    {
    ...
    receiver_adaptor<__receiver_ref<_SchedulerId, _SenderId, _ReceiverId>>
{
    ...
    {
        return make_env(execution::get_env(this->base()),
...
struct __operation
{
    ...
    std::variant<connect_result_t<schedule_result_t<_Scheduler>, __receiver_t>,
...
struct __sender
{
    ...
    using __receiver_t = __receiver<_SchedulerId, _SenderId, _ReceiverId>;
    template <class _ReceiverId>
    using __operation_t = __operation<_SchedulerId, _SenderId, _ReceiverId>;
...
struct transfer_just_t
{
    ...
    requires(!tag_invocable<transfer_just_t, _Scheduler, _Values...> ||
             !sender<tag_invoke_result_t<transfer_just_t, _Scheduler,
                                         _Values...>>) auto
        operator()(_Scheduler&& __sched, _Values&&... __vals) const
...
class __sender
{
    ...
    template <class _Env>
    using __completion_signatures = make_completion_signatures<
        _Sender, _Env,
    ...
    friend auto
        tag_invoke(connect_t, __sender&& __self, _Receiver&& __rcvr) noexcept(
...
template <class _Env>
using __env_t = make_env_t<_Env, with_t<get_stop_token_t, in_place_stop_token>>;
...
{
    using __non_values = __concat_completion_signatures_t<
        completion_signatures<set_error_t(std::exception_ptr), set_stopped_t()>,
        make_completion_signatures<_Senders, _Env, completion_signatures<>,
                                   __swallow_values>...>;
    using __values = __minvoke<__concat<__qf<set_value_t>>,
                               __value_types_of_t<_Senders, _Env, __q<__types>,
                                                  __single_or<__types<>>>...>;
    using __t = __if_c<(__sends<set_value_t, _Senders, _Env> && ...),
                       __minvoke2<__push_back<__q<completion_signatures>>,
                                  __non_values, __values>,
                       __non_values>;
};

template <class _CvrefEnv, class... _Senders>
using __completion_sigs = __t<__traits<__env_t<remove_cvref_t<_CvrefEnv>>,
                                       __member_t<_CvrefEnv, _Senders>...>>;

template <class _Traits>
using __sends_values = __bool<__v<typename _Traits::template __gather_sigs<
                                  set_value_t, __mconst<int>, __mcount>> != 0>;
...
struct __receiver :
    receiver_adaptor<__receiver<_CvrefReceiverId, _Index, _SenderIds...>>
{
    using _Receiver = __t<decay_t<_CvrefReceiverId>>;
    using _Traits =
        __completion_sigs<__member_t<_CvrefReceiverId, env_of_t<_Receiver>>,
                          __t<_SenderIds>...>;
    ...
    void set_value(_Values&&... __vals) && noexcept
    {
        if constexpr (__sends_values<_Traits>::value)
        {
            ...
            if (__op_state_->__state_ == __started)
            {
                try
                {
                    std::get<_Index>(__op_state_->__values_)
    ...
    void set_stopped() && noexcept
    {
        ...
        if (__op_state_->__state_.compare_exchange_strong(__expected,
                                                          __stopped))
        {
            __op_state_->__stop_source_.request_stop();
        }
...
struct __operation
{
    ...
    using __child_op_state =
        connect_result_t<__member_t<_WhenAll, _Sender>,
    ...
    void __complete() noexcept
    {
        ...
        switch (__state_.load(std::memory_order_relaxed))
        {
                    ...
                    std::apply(
                        [this](auto&... __opt_vals) -> void {
                            std::apply(
                                ...
                                std::tuple_cat(std::apply(
                                    [](auto&... __vals) {
                                        return std::tie(__vals...);
                                    },
                                    *__opt_vals)...));
                        },
                        __values_);
            ...
            case __error:
                std::visit(
                    [this](auto& __err) noexcept {
                        execution::set_error((_Receiver &&) __recvr_,
                                             std::move(__err));
                    },
                    __errors_);
                break;
    ...
    __operation(_WhenAll&& __when_all, _Receiver __rcvr,
        ...
        __child_states_{__conv{[&__when_all, this]() {
            return execution::connect(
    ...
    friend void tag_invoke(start_t, __operation& __self) noexcept
    {
        ...
        __self.__on_stop_.emplace(get_stop_token(get_env(__self.__recvr_)),
    ...
    using __child_values_tuple_t =
        __if<__sends_values<_Traits>,
             __minvoke<__q<std::tuple>,
                       __value_types_of_t<__t<_SenderIds>, __env_t<_Env>,
                                          __mcompose<__q1<std::optional>,
                                                     __q<__decayed_tuple>>,
                                          __single_or<void>>...>,
             __>;
    ...
    error_types_of_t<__sender<_SenderIds...>, __env_t<_Env>, __variant>
        __errors_{};
...
struct __sender
{
    ...
    explicit __sender(_Sndrs&&... __sndrs) : __sndrs_((_Sndrs &&) __sndrs...)
    {}
...
struct when_all_t
{
    ...
    requires tag_invocable<when_all_t, _Senders...> &&
        ...
        (sizeof...(_Senders) > 0) auto
        operator()(_Senders&&... __sndrs) const
    ...
    template <sender... _Senders>
    requires(!tag_invocable<when_all_t, _Senders...>) &&
        (sizeof...(_Senders) > 0) auto operator()(_Senders&&... __sndrs) const
        -> __sender<__x<decay_t<_Senders>>...>
    {
        return __sender<__x<decay_t<_Senders>>...>{(_Senders &&) __sndrs...};
    }
};
...
struct when_all_with_variant_t
{
    ...
    template <sender... _Senders>
    requires(!tag_invocable<when_all_with_variant_t, _Senders...>) &&
        (__callable<__into_variant_t, _Senders> && ...) auto
        operator()(_Senders&&... __sndrs) const
    {
...
struct transfer_when_all_t
{
    ...
    requires((!tag_invocable<transfer_when_all_t, _Sched, _Senders...>) ||
             (!sender<tag_invoke_result_t<transfer_when_all_t, _Sched,
                                          _Senders...>>)) auto
        operator()(_Sched&& __sched, _Senders&&... __sndrs) const
    {
...
struct transfer_when_all_with_variant_t
{
    ...
    requires tag_invocable<transfer_when_all_with_variant_t, _Sched,
                           _Senders...> &&
        sender<tag_invoke_result_t<transfer_when_all_with_variant_t, _Sched,
                                   _Senders...>>
...
struct __sender
{
    ...
    template <__decays_to<__sender> _Self, class _Env>
    friend auto tag_invoke(get_completion_signatures_t, _Self&&, _Env)
        -> make_completion_signatures<__member_t<_Self, _Sender>,
...
struct __env
{
    ...
    friend auto tag_invoke(execution::get_scheduler_t,
    ...
    friend auto tag_invoke(execution::get_delegatee_scheduler_t,
                           const __env& __self) noexcept
        -> execution::run_loop::__scheduler
    {
        return __self.__sched_;
...
using __sync_wait_result_t =
    execution::value_types_of_t<_Sender, __env, execution::__decayed_tuple,
                                __single_t>;
...
struct __receiver
{
    ...
    friend void tag_invoke(execution::set_value_t, __receiver&& __rcvr,
                           _As&&... __as) noexcept
    try
    {
    ...
    template <class _Error>
    friend void tag_invoke(execution::set_error_t, __receiver&& __rcvr,
                           _Error __err) noexcept
    {
        ...
        if constexpr (__decays_to<_Error, std::exception_ptr>)
            ...
            __rcvr.__state_->__data_.template emplace<2>(
                std::make_exception_ptr(std::system_error(__err)));
        else
            __rcvr.__state_->__data_.template emplace<2>(
                std::make_exception_ptr((_Error &&) __err));
    ...
    friend void tag_invoke(execution::set_stopped_t __d,
                           __receiver&& __rcvr) noexcept
    {
        __rcvr.__state_->__data_.template emplace<3>(__d);
        __rcvr.__loop_->finish();
    }
    friend __env tag_invoke(execution::get_env_t,
                            const __receiver& __rcvr) noexcept
    {
...
struct __state
{
    ...
    std::variant<std::monostate, _Tuple, std::exception_ptr,
...
struct sync_wait_t
{
    ...
    template <execution::__single_value_variant_sender<__impl::__env>
                  _Sender> // NOT TO SPEC
    requires execution::__tag_invocable_with_completion_scheduler<
        sync_wait_t, execution::set_value_t, _Sender>
        tag_invoke_result_t<sync_wait_t,
                            execution::__completion_scheduler_for<
                                _Sender, execution::set_value_t>,
                            _Sender>
    operator()(_Sender&& __sndr) const
        noexcept(nothrow_tag_invocable<sync_wait_t,
                                       execution::__completion_scheduler_for<
                                           _Sender, execution::set_value_t>,
                                       _Sender>)
    {
    ...
    requires(!execution::__tag_invocable_with_completion_scheduler<
             sync_wait_t, execution::set_value_t, _Sender>) &&
        tag_invocable<sync_wait_t, _Sender> tag_invoke_result_t<sync_wait_t,
                                                                _Sender>
        operator()(_Sender&& __sndr) const
    ...
    requires(!execution::__tag_invocable_with_completion_scheduler<
             sync_wait_t, execution::set_value_t, _Sender>) &&
        (!tag_invocable<sync_wait_t, _Sender>)&&execution::
            sender<_Sender, __impl::__env>&& execution::sender_to<
                _Sender, __impl::__receiver<__x<_Sender>>> auto
        operator()(_Sender&& __sndr) const
        -> std::optional<__impl::__sync_wait_result_t<_Sender>>
    {
        ...
        auto __op_state = execution::connect(
            (_Sender &&) __sndr,
...
struct sync_wait_with_variant_t
{
    ...
    requires execution::__tag_invocable_with_completion_scheduler<
        ...
        tag_invoke_result_t<sync_wait_with_variant_t,
                            ...
                            _Sender>
    operator()(_Sender&& __sndr) const
        noexcept(nothrow_tag_invocable<sync_wait_with_variant_t,
                                       execution::__completion_scheduler_for<
                                           _Sender, execution::set_value_t>,
                                       _Sender>)
    {
        static_assert(
            std::is_same_v<
                tag_invoke_result_t<sync_wait_with_variant_t,
                                    ...
                                    _Sender>,
                std::optional<
                    __impl::__sync_wait_with_variant_result_t<_Sender>>>,
    ...
    requires(!execution::__tag_invocable_with_completion_scheduler<
             sync_wait_with_variant_t, execution::set_value_t, _Sender>) &&
        tag_invocable<sync_wait_with_variant_t, _Sender> tag_invoke_result_t<
    ...
    requires(!execution::__tag_invocable_with_completion_scheduler<
             sync_wait_with_variant_t, execution::set_value_t, _Sender>) &&
        (!tag_invocable<sync_wait_with_variant_t, _Sender>)&&invocable<
            sync_wait_t, __impl::__into_variant_result_t<_Sender>> std::
...
struct __operation
{
    ...
    using _State =
        __if_c<same_as<_Result, void>, __void, std::optional<_Result>>;
    ...
    friend void tag_invoke(start_t, __operation& __self) noexcept
    {
        __self.__state_.emplace(__conv{
            [&]() noexcept { return ((_Fn &&) __self.__fn_)(__self.__ctx_); }});
    }
};
...
struct __sender
{
    ...
    & > &&constructible_from<_Fn, __member_t<_Self, _Fn>>&&
              constructible_from<_Args, __member_t<_Self, _Args>> friend auto
              ...
              -> __operation<__x<decay_t<_Receiver>>, _FnId, _ArgsId>
    {
        ...
        return {{(_Receiver &&) __rcvr, ((_Self &&) __self).__args_},
                ((_Self &&) __self).__fn_};
    }
};
