...
std::string busName = "xyz.openbmc_project.Testing" + std::to_string(rand());

sdbusplus::async::task<void> constructInterfaces(sdbusplus::async::context& ctx)
{
    ...
    sdbusplus::async::server_t<A, sdbusplus::aserver::server::Test> a3(
        ctx, "/3", props);
...
int main()
{
    ...
    ctx.spawn(constructInterfaces(ctx));
