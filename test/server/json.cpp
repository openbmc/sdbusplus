...
static_assert(sdbusplus::exception::has_interface_type<
...
TEST(JsonEvent, ExtendCanBeChained)
{
    using Event = sdbusplus::event::server::Test::EnumObjectPathEvent;
    using Props = sdbusplus::common::server::Test2::properties_t;

    const auto makeEvent = [] {
        return Event(Event::metadata_t<"ENUM_VALUE">{"ENUM_VALUE"},
                     EnumOne::OneA,
                     Event::metadata_t<"OBJECT_PATH">{"OBJECT_PATH"},
                     sdbusplus::object_path("/a/b/"));
    };

    // rvalue: the temporary is moved through the chain.
    Props a;
    a.new_value = 1;
    a.other_value = 2;
    auto j = makeEvent().extend(a).extend(a).to_json();
    const auto& self = j.at(Event::errName);
    EXPECT_EQ(self.at("_EXTENSIONS").at("server.Test2").at("NewValue"), 1);

    // lvalue: both calls return an lvalue reference to the same object.
    Event e = makeEvent();
    auto& e1 = e.extend(a);
    auto& e2 = e1.extend(a);
    EXPECT_EQ(&e1, &e2);
