...
def find_yaml(directory):
    result = [
        ...
        for y in glob(os.path.join(x[0], "*.interface.yaml"))
    ]
    return result
...
class FullTree(NamedElement, Renderer):
    ...
    def load(name, rootdir, schemadir):
        ...
        for intf in interfaceNames:
            filename = os.path.join(rootdir, intf + ".interface.yaml")
    ...
    def __init__(self, **kwargs):
        self.interfaces = [
            Interface(**p) for p in kwargs.pop("interfaces", [])
        ]
        ...
        for i, path in enumerate(self.paths):
            ...
            if segments[0] not in self.structNodes:
                self.structNodes[segments[0]] = StructNode(
                    intf=intf, origSegments=segments, depth=1
                )
