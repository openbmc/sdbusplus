...
void runTests() {
  ...
  // Test multiple strings, various forms.
  {
    ...
    struct verify {
      static void op(sd_bus_message* m) {
        int32_t a = 0, b = 0;
        const char *s0 = nullptr, *s1 = nullptr, *s2 = nullptr, *s3 = nullptr,
                   *s4 = nullptr, *s5 = nullptr, *s6 = nullptr;
        sd_bus_message_read(m, "isssssssi", &a, &s0, &s1, &s2, &s3, &s4, &s5,
                            &s6, &b);
        assert(a == 1);
        assert(b == 5);
  ...
  // Test vector.
  {
    ...
    struct verify {
      static void op(sd_bus_message* m) {
        ...
        auto rc = sd_bus_message_enter_container(m, SD_BUS_TYPE_ARRAY, "s");
        assert(0 <= rc);
  ...
  // Test map.
  {
    ...
    struct verify {
      static void op(sd_bus_message* m) {
        ...
        auto rc = sd_bus_message_enter_container(m, SD_BUS_TYPE_ARRAY, "{si}");
        assert(0 <= rc);

        rc = sd_bus_message_enter_container(m, SD_BUS_TYPE_DICT_ENTRY, "si");
        assert(0 <= rc);
  ...
  // Test tuple.
  {
    ...
    struct verify {
      static void op(sd_bus_message* m) {
        ...
        auto rc = sd_bus_message_enter_container(m, SD_BUS_TYPE_STRUCT, "ids");
        assert(0 <= rc);
