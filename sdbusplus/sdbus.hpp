...
class SdBusInterface
{
 public:
  ...
  //virtual int sd_bus_message_append(
  //    sd_bus_message *m, const char *types, ...) = 0;

  virtual int sd_bus_message_append_basic(
      sd_bus_message* message, char type, const void* value) = 0;
  ...
  virtual int sd_bus_message_enter_container(
      sd_bus_message *m, char type, const char *contents) = 0;

  virtual int sd_bus_message_exit_container(sd_bus_message *m) = 0;

  virtual sd_bus* sd_bus_message_get_bus(sd_bus_message *m) = 0;
  virtual int sd_bus_message_get_cookie(
      sd_bus_message *m, uint64_t *cookie) = 0;
  ...
  virtual const char *sd_bus_message_get_signature(
      sd_bus_message *m, int complete) = 0;

  virtual int sd_bus_message_is_method_call(
      sd_bus_message *m, const char *interface, const char *member) = 0;
  virtual int sd_bus_message_is_method_error(
      sd_bus_message *m, const char *name) = 0;
  virtual int sd_bus_message_is_signal(
      sd_bus_message *m, const char *interface, const char *member) = 0;

  virtual int sd_bus_message_new_method_return(
      sd_bus_message *call, sd_bus_message **m) = 0;

  virtual int sd_bus_message_read_basic(
      sd_bus_message *m, char type, void *p) = 0;

  virtual int sd_bus_message_skip(sd_bus_message *m, const char *types) = 0;

  virtual int sd_bus_message_verify_type(
      sd_bus_message *m, char type, const char *contents) = 0;

  virtual int sd_bus_send(
      sd_bus *bus, sd_bus_message *m, uint64_t *cookie) = 0;

  virtual int sd_bus_message_open_container(
      sd_bus_message *m, char type, const char *contents) = 0;
};
...
class SdBusImpl : public SdBusInterface
{
 public:
  ...
  //int sd_bus_message_append(sd_bus_message *m, const char *types, ...) override
  //{
  //    return ::sd_bus_message_append(m, types, ...);
  //}

  int sd_bus_message_append_basic(
      sd_bus_message* message, char type, const void* value) override
  {
  ...
  int sd_bus_message_enter_container(
      sd_bus_message *m, char type, const char *contents) override
  {
  ...
  int sd_bus_message_new_method_return(sd_bus_message *call, sd_bus_message **m) override
  {
      return ::sd_bus_message_new_method_return(call, m);
  }

  int sd_bus_message_read_basic(sd_bus_message *m, char type, void *p) override
  {
  ...
  int sd_bus_message_verify_type(sd_bus_message *m, char type, const char *contents) override
  {
