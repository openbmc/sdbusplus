...
class dbus_interface
{
  public:
    ...
    bool emits_change(const std::string& propertyName) const
    {
        auto it = std::find_if(
            vtable_.begin(), vtable_.end(), [&propertyName](const auto& item) {
                return item.type == _SD_BUS_VTABLE_PROPERTY &&
                       item.x.property.member == propertyName;
            });
        if (it != vtable_.end())
        {
            return it->flags & vtable::property_::emits_change;
        }
        return false;
    }
