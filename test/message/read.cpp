...
{
  public:
    explicit AssignReadArrayVal(const T& val) : val_(val) {}
    ...
    void operator()(sd_bus_message*, char, const void** p, size_t* sz) const
    {
        ...
        *sz = val_.size() * elementSize;
        auto* data = new typename T::value_type[val_.size()];

        std::copy(val_.begin(), val_.end(), data);
  ...
  protected:
    ...
    void expect_read_array(char type, T val)
    {
        EXPECT_CALL(mock, sd_bus_message_read_array(nullptr, type, testing::_,
                                                    testing::_))
            .WillOnce(
                DoAll(testing::Invoke(AssignReadArrayVal<T>(val)), Return(0)));
    }
