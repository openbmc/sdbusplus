...
struct __sender
{
    using completion_signatures = execution::completion_signatures<
        execution::set_value_t(), execution::set_error_t(std::exception_ptr)>;
                     ...
                     -> __operation<__x<remove_cvref_t<_Receiver>>>
    {
        return __operation<__x<remove_cvref_t<_Receiver>>>{
            *__self.__evt_, (_Receiver &&) __rcvr};
    }
...
struct __async_manual_reset_event
{
    __async_manual_reset_event() noexcept : __async_manual_reset_event(false) {}
    ...
    void reset() noexcept
    {
        ...
        // spuriously; if it fails, it means it wasn't previously in the
        // signalled state so resetting is a no-op.
        (void)__state_.compare_exchange_strong(__old_state, nullptr,
  ...
  private:
    ...
    // note: this is a static method that takes __evt *second* because the
    // caller
    ...
    static void
        __start_or_wait_(__op_base& __op,
                         const __async_manual_reset_event& __evt) noexcept;
};
...
struct __op_base : __immovable
{
    ...
    //
    // note: __next_ and __set_value_ are the first two members because this
    // ordering
    //       leads to smaller code than others; on ARM, the first two members
    //       can be loaded into a pair of registers in one instruction, which
    //       turns out to be important in both __async_manual_reset_event::set()
    //       and
    //       __start_or_wait_().
    __op_base* __next_;
    ...
    explicit __op_base(const __async_manual_reset_event* __evt,
    ...
    private receiver_adaptor<__receiver<_ReceiverId>, __t<_ReceiverId>>
{
    ...
    auto get_env() const& -> make_env_t<
        env_of_t<_Receiver>, with_t<get_stop_token_t, never_stop_token>>
    {
...
template <class _Receiver>
auto __connect_as_unstoppable(_Receiver&& __rcvr) noexcept(
    __nothrow_connectable<schedule_result_t<__scheduler_from_env_of<_Receiver>>,
                          __receiver<__x<_Receiver>>>)
    -> connect_result_t<schedule_result_t<__scheduler_from_env_of<_Receiver>>,
                        __receiver<__x<_Receiver>>>
{
    return connect(schedule(get_scheduler(get_env(__rcvr))),
...
struct __operation : private __op_base
{
    ...
    explicit __operation(
        ...
        _Receiver
            __rcvr) noexcept(noexcept(__connect_as_unstoppable(std::
                                                                   move(
                                                                       __rcvr)))) :
...
inline void __async_manual_reset_event::set() noexcept
{
    ...
    // been signalled
    void* __top = __state_.exchange(__signalled_state,
                                    std::memory_order_acq_rel);
...
inline void __async_manual_reset_event::__start_or_wait_(
    __op_base& __op, const __async_manual_reset_event& __evt) noexcept
{
    ...
    do
    {
        ...
        __top, static_cast<void*>(&__op), std::memory_order_release,
...
class __nest_operation : __immovable
{
    ...
    {
        if (!std::exchange(__op_state.__nested_, false))
        {
            std::terminate();
        }
  ...
  public:
    ...
    explicit __nest_operation(_Receiver2&& __rcvr, _Sender&& __sndr,
                              async_scope* __scope) :
        __rcvr_((_Receiver2 &&) __rcvr),
        __scope_(__scope), __forward_scope_(__get_stop_token_(this->__scope_),
                                            __forward_stopped{this}),
        __forward_consumer_(get_stop_token(get_env(this->__rcvr_)),
                            __forward_stopped{this}),
        __op_(connect(std::move(__sndr),
                      __nest_receiver<_SenderId, _ReceiverId>{this, __scope})),
        __nested_(true)
    {}
};
} // namespace __impl
    ...
    __receiver_base
{
    ...
    {
        static_assert(
            same_as<_State, __impl::__nest_operation<_SenderId, _ReceiverId>>);
    }
  ...
  private:
    ...
    __impl::__nest_operation<_SenderId, _ReceiverId>& get_state() const
    {
        return *reinterpret_cast<
            __impl::__nest_operation<_SenderId, _ReceiverId>*>(__op_);
    }
    ...
    auto get_env()
        const& -> make_env_t<with_t<get_stop_token_t, in_place_stop_token>>
    {
        return make_env(
            with(get_stop_token, get_state().__stop_source_.get_token()));
    }
};
...
class __nest
{
    ...
    explicit __nest(_Sender2&& __sndr, async_scope* __scope) noexcept :
        __sndr_((_Sender2 &&) __sndr), __scope_(__scope)
    {}
        ...
        tag_invoke(connect_t, __nest&& __self, _Receiver&& __rcvr)
    {
        try
        {
            return __impl::__nest_operation<_SenderId, __x<decay_t<_Receiver>>>{
                (_Receiver &&) __rcvr, std::move(__self.__sndr_),
                __self.__scope_};
        }
        catch (...)
        {
...
class __operation : __subscription
{
    ...
    void __complete_() noexcept final override
    try
    {
        static_assert(sender_to<_Sender, _Receiver>);
        if (__state_->__data_.index() == 2 ||
            get_stop_token(get_env(__rcvr_)).stop_requested())
        {
  ...
  public:
    template <class _Receiver2>
    explicit __operation(_Receiver2&& __rcvr,
                         std::unique_ptr<__future_state<_Sender>> __state) :
        __rcvr_((_Receiver2 &&) __rcvr),
...
using __future_result_t =
    execution::value_types_of_t<_Sender, __empty_env,
                                execution::__decayed_tuple, __single_t>;
} // namespace __impl
    ...
    __receiver_base
{
    ...
    explicit __future_receiver(_State* __state, async_scope* __scope) noexcept :
        receiver_adaptor<__future_receiver<_SenderId>>{}, __receiver_base{
                                                              __state, __scope}
    {
...
class __future_state : std::enable_shared_from_this<__future_state<_Sender>>
{
    ...
    std::variant<std::monostate, __impl::__future_result_t<_Sender>,
...
{
template <class _SenderId, class _ReceiverId>
inline void __operation<_SenderId, _ReceiverId>::__start_() noexcept
try
{
    if (!!__state_)
    {
...
template <sender _Sender>
inline void __future_state<_Sender>::__push_back_(
    __impl::__subscription* __subscription)
{
...
struct async_scope
{
  ...
  public:
    async_scope() noexcept :
        __end_of_scope_callback_(__stop_source_.get_token(),
                                 __end_of_scope_callback{this}),
        __evt_(true)
    {}
    ...
    ~async_scope()
    {
        ...
        [[maybe_unused]] auto __state =
            __op_state_.load(std::memory_order_relaxed);
    ...
    void spawn(_Sender&& __sndr)
    {
        ...
        __op_to_start->emplace(__conv{[&] {
            return connect((_Sender &&) __sndr,
                           __receiver<__x<remove_cvref_t<_Sender>>>{
                               __op_to_start.get(), this});
        }});

        // At this point, the rest of the function is noexcept, but
        // __op_to_start's destructor is no longer enough to properly clean up
        // because it won't invoke destruct(). We need to ensure that we either
        // call destruct() ourselves or complete the operation so *it* can call
        // destruct().
    ...
    template <class _Sender>
        requires sender_to<_Sender,
                           __future_receiver<__x<remove_cvref_t<_Sender>>>>
  ...
  private:
    ...
    friend void __record_start_(async_scope* __scope) noexcept
    {
        ...
        do
        {
            ...
            __op_state, __op_state + 2, std::memory_order_relaxed));
    ...
    {
        auto __old_state =
            __scope->__op_state_.fetch_sub(2, std::memory_order_release);
