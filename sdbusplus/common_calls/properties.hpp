...
using PropertyMap = std::map<std::string, std::variant<Properties...>>;

template <typename... Properties>
auto getAll(bus::bus& bus, const char* service, const char* object)
{
    using Req = typed::Types<std::string>;
    using Rsp = typed::Types<PropertyMap<Properties...>>;
    return typed::MethodCall<Req, Rsp>(bus.new_method_call(
        service, object, "org.freedesktop.DBus.Properties", "GetAll"));
}

template <typename... Properties>
std::map<std::string, std::variant<Properties...>>
    getAll(bus::bus& bus, const char* service, const char* object,
           const char* interface)
{
    PropertyMap<Properties...> ret;
