...
struct NrNonDbusArgs;

template <>
struct NrNonDbusArgs<std::tuple<>>
{
...
struct NrNonDbusArgs<std::tuple<FirstArg, OtherArgs...>>
{
    constexpr static std::size_t size()
    {
        ...
        {
            return 1 + NrNonDbusArgs<std::tuple<OtherArgs...>>::size();
        }
        else
        {
            return NrNonDbusArgs<std::tuple<OtherArgs...>>::size();
        }
    }
};
} // namespace details
...
class callback_method_instance : public callback
{
  ...
  private:
    ...
    template <typename T>
    std::enable_if_t<callbackWantsMessage<T>, int>
  ...
  private:
    ...
    void expandCall(boost::asio::yield_context yield, message::message& m)
    {
        using DbusTupleType = typename utility::strip_first_n_args<
            details::NrNonDbusArgs<CallbackSignature>::size(),
            CallbackSignature>::type;
        DbusTupleType dbusArgs;
        if (!utility::read_into_tuple(dbusArgs, m))
        {
        ...
        auto ret = m.new_method_return();
        std::vector<InputTupleType> inputArgs;
        if constexpr (callbackWantsMessage<CallbackType>)
        {
            inputArgs.push_back(
                std::tuple_cat(std::forward_as_tuple(yield),
                               std::forward_as_tuple(std::move(m)), dbusArgs));
        }
        else
        {
            inputArgs.push_back(
                std::tuple_cat(std::forward_as_tuple(yield), dbusArgs));
        }
        callFunction<ResultType>(ret, inputArgs[0]);
        ret.method_return();
    };
};
  ...
  public:
    ...
    bool register_method(const std::string& name, CallbackType&& handler)
    {
        ...
        using CallbackSignature = typename utility::strip_first_n_args<
            details::NrNonDbusArgs<ActualSignature>::size(),
