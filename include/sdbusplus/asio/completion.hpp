...
{
  public:
    // The method's return types, as a tuple; used to derive the D-Bus output
    // signature when the method is registered.
...
class completion_method_instance
{
  private:
    ...
    static_assert(
        requires { typename CompletionType::result_type; },
