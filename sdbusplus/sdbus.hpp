...
class SdBusInterface
{
    public:
        ...
        virtual int sd_bus_list_names(sd_bus *bus, char ***acquired, char ***activatable) = 0;
...
class SdBusImpl : public SdBusInterface
{
  public:
    ...
    {
        return ::sd_bus_add_object_vtable(bus, slot, path,
    ...
    int sd_bus_call(sd_bus *bus, sd_bus_message *m, uint64_t usec, sd_bus_error *ret_error, sd_bus_message **reply) override
    {
    ...
    int sd_bus_list_names(sd_bus *bus, char ***acquired, char ***activatable) override
    {
