...
namespace ${s}
{
    ...
    % endfor
        ...
        % if interface.properties : using PropertiesVariant =
...
${ s.cpp_prototype(loader, interface=interface, ptype='header') }
    ...
    % endfor
         ...
         */
        void
        setPropertyByName(const std::string& name,
...
${ m.cpp_prototype(loader, interface=interface, ptype='callback-header') }
    ...
    static int _callback_set_${p.name}(sd_bus*, const char*, const char*,
    ...
    sdbusplus::server::interface::interface _$ {
      "_".join(interface.name.split('.'))
    }
    _interface;
    ...
    _${p.camelCase} = % if p.is_enum() : $ { p.cppTypeParam(interface.name) }
    :: % endif ${p.defaultValue};
    % else : $ {
      p.cppTypeParam(interface.name)
    }
    _${p.camelCase} {};
