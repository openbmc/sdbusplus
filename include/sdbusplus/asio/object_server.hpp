...
{
    std::string name;
    std::function<bool(message_t&)> on_get;
...
{
    std::string name;
    std::function<bool(message_t&)> call;
...
struct callback_method_instance
{
    bool operator()(message_t& m)
    {
        ...
        if constexpr (callbackWantsMessage<CallbackType>)
        {
            ...
            apply(m, inputArgs, func_);
        }
        else
        {
            apply(m, dbusArgs, func_);
        }
        ret.method_return();
        return true;
    }
...
template <typename CallbackType>
struct coroutine_method_instance
{
