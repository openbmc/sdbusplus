 ...
 */
template <typename... Args> void read(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Args&&... args);
...
{
    template <typename T> static void op(sdbusplus::SdBusInterface* intf, sd_bus_message* m, T&& s)
    {
        constexpr auto dbusType = std::get<0>(types::type_id<T>());
...
{
    template <typename S> static void op(sdbusplus::SdBusInterface* intf, sd_bus_message* m, S&& s)
    {
...
{
    template <typename T> static void op(sdbusplus::SdBusInterface* intf, sd_bus_message* m, T&& b)
    {
        constexpr auto dbusType = std::get<0>(types::type_id<T>());
        int i = 0;
...
template <typename... Args> struct read_single<std::tuple<Args...>>
{
    ...
    template <typename S> static void op(sdbusplus::SdBusInterface* intf, sd_bus_message* m, S&& s)
    {
        ...
        intf->sd_bus_message_enter_container(m, SD_BUS_TYPE_STRUCT, dbusType.data());
        _op(intf, m, std::forward<S>(s), std::make_index_sequence<sizeof...(Args)>());
        intf->sd_bus_message_exit_container(m);
    }
};
...
template <typename... Args> struct read_single<variant<Args...>>
{
    ...
    static void read(sdbusplus::SdBusInterface* intf, sd_bus_message* m, S&& s)
    {
        ...
        auto rc =
            intf->sd_bus_message_verify_type(m, SD_BUS_TYPE_VARIANT, dbusType.data());
        if (0 >= rc)
        {
        ...
        intf->sd_bus_message_enter_container(m, SD_BUS_TYPE_VARIANT, dbusType.data());
    ...
    template <typename S> static void read(sdbusplus::SdBusInterface* intf, sd_bus_message* m, S&& s)
    {
...
/** @brief Read a tuple of content from the sd_bus_message. [UNUSED]
 *
...
void read_tuple(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Tuple&& t, std::index_sequence<I...>)
{
    auto dbusTypes =
        utility::tuple_to_array(types::type_id<decltype(std::get<I>(t))...>());

    // c++14 we can just do this.
    //((sdbusplus::message::read(intf, m, std::get<I>(t))), ...);

    sd_bus_message_read(m, dbusTypes.data(), &std::get<I>(t)...);
}

template <typename T>
static void tuple_item_read(sdbusplus::SdBusInterface *intf, sd_bus_message *m, T&&t) {
  sdbusplus::message::read(intf, m, t);
}
...
struct ReadHelper {
  ...
  static void op(sdbusplus::SdBusInterface* intf, sd_bus_message* m, std::tuple<Fields...> field_tuple) {
    ReadHelper<Index - 1>::op(intf, m, field_tuple);

    tuple_item_read(intf, m, std::get<Index - 1>(field_tuple));
  }
};
    ...
    read_tuple(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Tuple&& t)
{
    ...
    read_single_t<itemType>::op(intf, m, std::forward<itemType>(std::get<0>(t)));
}
...
std::enable_if_t<
    ...
    read_grouping(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Tuple&& t, Arg&& arg);
/** @brief Group a sequence of C++ types for reading from an sd_bus_message.
 *  @tparam Tuple - A tuple of previously analyzed types.
 *  @tparam Arg - The argument to analyze for grouping.
 *
...
std::enable_if_t<
    can_read_multiple<types::details::type_id_downcast_t<Arg>>::value>
    read_grouping(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Tuple&& t, Arg&& arg, Rest&&... rest);
/** @brief Group a sequence of C++ types for reading from an sd_bus_message.
 *  @tparam Tuple - A tuple of previously analyzed types.
 *  @tparam Arg - The argument to analyze for grouping.
 *  @tparam Rest - The remaining arguments left to analyze.
...
template <typename... Args> void read(sdbusplus::SdBusInterface* intf, sd_bus_message* m, Args&&... args)
{
    details::read_grouping(intf, m, std::make_tuple(), std::forward<Args>(args)...);
}
