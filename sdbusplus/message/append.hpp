...
void append(sd_bus_message* m, Args&&... args);
...
struct append_single {
  ...
  static void op(sd_bus_message* m, T&& t) {
    ...
    sd_bus_message_append_basic(m, dbusType, address_of(std::forward<T>(t)));
  }
};
...
template <typename T>
struct append_single<std::vector<T>> {
  template <typename S>
  static void op(sd_bus_message* m, S&& s) {
...
struct append_single<std::pair<T1, T2>> {
  ...
  static void op(sd_bus_message* m, S&& s) {
    ...
    sd_bus_message_open_container(m, SD_BUS_TYPE_DICT_ENTRY, dbusType.data());
...
template <typename T1, typename T2>
struct append_single<std::map<T1, T2>> {
  template <typename S>
  static void op(sd_bus_message* m, S&& s) {
    constexpr auto dbusType = utility::tuple_to_array(
...
struct append_single<variant<Args...>> {
  ...
  static void op(sd_bus_message* m, S&& s) {
    auto apply = [m](auto&& arg) {
      ...
      sd_bus_message_open_container(m, SD_BUS_TYPE_VARIANT, dbusType.data());
...
std::enable_if_t<2 <= std::tuple_size<Tuple>::value> append_tuple(
 ...
 */
template <typename Tuple>
std::enable_if_t<1 == std::tuple_size<Tuple>::value> append_tuple(
    sd_bus_message* m, Tuple&& t) {
