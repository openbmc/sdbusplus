...
Daemon::Daemon(std::vector<const char*> argvList, const std::string oPath,
               const std::string ePath) :
    stdoutPath(oPath),
    stderrPath(ePath), pid(-1), started(false), argv(std::move(argvList))
{
    buildCmd();
...
void Daemon::postTerminate()
{
    if (getpgid(getPid()) == 0)
    {
        ...
        {
            std::cout << "daemon that started with command: " << cmd
                      << std::endl
