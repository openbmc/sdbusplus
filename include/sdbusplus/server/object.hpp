    ...
    private sdbusplus::bus::details::bus_friend
{
    ...
    enum class action
    {
        ...
        /** no initial emit, sd_bus_emit_object_removed on destruct */
        defer_emit,
    ...
    ~object()
    {
        if (__sdbusplus_server_object_emitremoved != action::emit_no_signals)
        {
    ...
    {
        if (__sdbusplus_server_object_emitremoved == action::defer_emit)
        {
  ...
  private:
    ...
    action __sdbusplus_server_object_emitremoved = action::defer_emit;
    ...
    void check_action(action act)
    {
        switch (act)
        {
            ...
            case action::emit_interface_added:
                ...
                __sdbusplus_server_object_emitremoved = action::emit_no_signals;
                break;

            case action::defer_emit:
                // Even though we are defering the added signal (or maybe never
                // sending it because we haven't claimed a busname yet), we
                // still want to emit the object_removed signal on destruction.
                //
                // It is a common pattern on daemon start-up to not emit signals
                // for static objects and then desire to have the removal
                // signals emitted.
                __sdbusplus_server_object_emitremoved = action::defer_emit;
                break;
