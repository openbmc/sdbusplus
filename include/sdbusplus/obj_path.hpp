...
struct obj_path
{
    ...
    constexpr auto operator/(sdbusplus::obj_path<S2> o2)
        requires(S.size() == 2)
    ...
    template <fixed_string S2>
    constexpr auto operator/(sdbusplus::obj_path<S2>)
        requires(S.size() > 2)
    {
        ...
        return sdbusplus::obj_path<fixed>();
    }
    ...
    constexpr std::string operator/(std::string_view extId) const
    {
        ...
        if (extId.empty() ||
            (!message::details::pathShouldEscape(extId[0]) &&
             std::none_of(extId.begin() + 1, extId.end(),
                          message::details::pathRequiresEscape)))
        {
            str.append(extId);
            return str;
        }
        message::details::pathAppendEscape(str, extId[0]);
        for (auto c : extId.substr(1))
        {
            if (message::details::pathShouldEscape(c))
            {
                message::details::pathAppendEscape(str, c);
            }
            else
            {
                str.append(1, c);
    ...
    template <fixed_string S2>
    constexpr bool operator==(const sdbusplus::obj_path<S2>& _)
    {
        return S == S2;
    }
};
