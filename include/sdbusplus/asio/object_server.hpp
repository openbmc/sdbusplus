...
#include <sdbusplus/asio/deferred_reply.hpp>
...
class dbus_interface
{
  public:
                                  ...
                                  decltype(vtable_t::flags) flags = 0)
    {
        ...
        static_assert(
            std::is_same_v<FirstArg, deferred_reply>,
            "register_deferred_method: handler's first parameter must be "
            "sdbusplus::asio::deferred_reply");
        static_assert(
        ...
        std::function<int(message_t&)> func =
            deferred_callback_method_instance<CallbackType>(std::move(handler));
        return addMethodCallback<InputTupleType>(name, std::move(func),
                                                 resultType.data(), flags);
    }
  ...
  private:
    ...
    // register_deferred_method, which differ only in how they build func and
